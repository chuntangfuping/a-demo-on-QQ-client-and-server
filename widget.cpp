...
void Widget::sndMsg(MsgType type, QString srvaddr)
{
    ...
    case Msg :
        if (ui->msgTxtEdit->toPlainText() == "") {
            QMessageBox::warning(0,tr("warning"),tr("don't transmit a empty content"),QMessageBox::Ok);
            return;
        }
        out << address << getMsg();
...
void Widget::usrEnter(QString usrname, QString ipaddr)
{
    ...
    if (isEmpty) {
        ...
        ui->usrNumLbl->setText(tr("online people number：%1").arg(ui->usrTblWidget->rowCount()));
...
void Widget::usrLeft(QString usrname, QString time)
{
    ...
    ui->msgBrowser->setTextColor(Qt::gray);
    ui->msgBrowser->setCurrentFont(QFont("Times New Roman", 10));
    ui->msgBrowser->append(tr("Between %1 to %2 leave").arg(usrname).arg(time));
    ui->usrNumLbl->setText(tr("online people number：%1").arg(ui->usrTblWidget->rowCount()));
}
...
void Widget::on_sendTBtn_clicked()
{
    ...
    {
        QMessageBox::warning(0, tr("choose customer"),tr("please choose target user"), QMessageBox::Ok);
        return;
    }
    srv->show();
    srv->initSrv();
...
void Widget::hasPendingFile(QString usrname, QString srvaddr,QString clntaddr, QString filename)
{
    ...
    if(ipAddr == clntaddr)
    {
        int btn = QMessageBox::information(this,tr("receive file"),tr("file from %1(%2)：where %3 accept or not").arg(usrname).arg(srvaddr).arg(filename),QMessageBox::Yes,QMessageBox::No);
